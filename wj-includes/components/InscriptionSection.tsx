...
export default function InscriptionSection({ onHoverCta }: InscriptionSectionProps) {
  ...
  return (
    <section
      ...
      className="relative min-h-screen flex items-center border-t border-ink/10 py-24 bg-abyss overflow-hidden"
    >
      <div className="absolute inset-0 grid-bg opacity-40 pointer-events-none" />
      <div
        aria-hidden="true"
        className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 size-[480px] rounded-full blur-[120px] opacity-20 pointer-events-none"
        style={{ background: "#FF6300" }}
      />

      <div className="relative max-w-4xl mx-auto px-6 md:px-12 text-center">
        <p className="inline-flex items-center gap-2 font-mono text-xs tracking-[0.3em] text-ember-ink uppercase mb-6">
        ...
        <p className="text-ink/50 max-w-xl mx-auto mb-8">{INSCRIPCION.parrafo}</p>

        <p className="font-mono text-xs mb-10">
          <span className="text-amber-ink">{INSCRIPCION.datoCupos}</span>
          <span className="text-ink/40">{INSCRIPCION.datoCuposEtiqueta}</span>
          <span className="text-ink/40"> · </span>
          <span className="text-amber-ink">{INSCRIPCION.datoModalidad}</span>
          <span className="text-ink/40">{INSCRIPCION.datoModalidadEtiqueta}</span>
          <span className="text-amber-ink">{INSCRIPCION.datoFecha}</span>
          <span className="text-ink/40">{INSCRIPCION.datoFechaEtiqueta}</span>
        </p>
        ...
        {hasForm ? (
          ...
          <span
            ...
            className="inline-flex items-center gap-3 rounded-full bg-ember px-8 py-4
                       font-display text-base font-bold text-on-accent pointer-events-none opacity-40"
          >
            {INSCRIPCION.botonTexto}
        ...
        <p className="text-ink/30 text-xs mt-4">
